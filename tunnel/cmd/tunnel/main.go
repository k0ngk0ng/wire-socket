...
import (
	...
	"wire-socket-tunnel/internal/api"
	"wire-socket-tunnel/internal/database"
	"wire-socket-tunnel/internal/nat"
	"wire-socket-tunnel/internal/wireguard"
	"wire-socket-tunnel/internal/wstunnel"
...
func main() {
	...
	if *register {
		tempHandler := api.NewAuthHandler(db, wgManager, authConfig)
	...
	// Setup API routes
	router := api.NewRouter(db, wgManager, natManager, authConfig, config.WireGuard.DeviceName)
...
func startTunnel(config *Config, wgManager *wireguard.Manager) {
	tunnelConfig := wstunnel.Config{
		...
		TargetAddr: "127.0.0.1:" + itoa(config.WireGuard.ListenPort),
